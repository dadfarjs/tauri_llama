...
const AppSidebar = () => {
  const { setOpenDialog } = useSettingDialogStore();
  return (
    <div className="bg-black flex flex-col items-center justify-between p-3 h-screen">
      <div className="flex flex-col justify-start w-full h-full overflow-y-scroll">
        <div className="w-full">
          <div className="w-full max-w-md py-4">
            <Field>
              <Label className="text-sm/6 font-medium text-white">Select AI</Label>
              <Description className="text-sm/6 text-white/50">Select a AI for chat</Description>
        ...
        <div>
          ...
          <div className="text-[#ececf1] text-sm space-y-4 mt-4 w-full overflow-y-scroll">
      ...
      <div className="sticky bottom-0 flex flex-col w-full bg-black hover:bg-primary px-1 py-2 rounded-md space-y-4">
        {/* account */}
        <div
          className="flex items-center space-x-2 cursor-pointer"
          onClick={() => setOpenDialog(true)}
        >
          <HiCog color="white" size={35} />
          <p className="text-[#ececf1]">Setting AI</p>
        </div>
      </div>
    </div>
  );
